...
export function VentasBoard({ cards, onCardClick }: Props) {
  const ventasCards = cards.filter(c => c.column_status === 'vendido')
        ...
        return (
          <div key={month.toISOString()}
            ...
            <div className="px-4 py-3 bg-emerald-50 border-b border-slate-300 flex items-center gap-2.5">
              ...
              {monthCards.length > 0 && (
                <span className="text-[12px] font-bold text-white bg-emerald-500 min-w-[22px] h-[22px] rounded-full flex items-center justify-center">
                  {monthCards.length}
                </span>
              )}
            </div>
            <div className="flex-1 overflow-y-auto px-2.5 py-2.5 column-scroll bg-slate-100/80">
              ...
              {monthCards.map(card => (
                <div key={card.id} onClick={() => onCardClick(card)}
                  className="bg-white rounded-lg border border-slate-300 border-l-[4px] border-l-emerald-500
                  ...
                  <p className="text-[14px] text-emerald-600 font-semibold text-right mt-1">
