    ...
    <div className={`flex flex-col min-w-0 flex-1 h-full border-r border-slate-300 last:border-r-0 border-t-[3px] ${accentBorder}`}>
      <div className={`flex items-center justify-between px-4 py-3 ${headerBg} border-b border-slate-300`}>
        ...
        <button onClick={onAddClick}
          className="bg-slate-800 hover:bg-slate-700 text-white px-4 py-2 rounded-lg text-[14px] font-semibold transition-all duration-150 active:scale-[0.97] shadow-sm hover:shadow-md">
          + Agregar
        </button>
      </div>
      <Droppable droppableId={id}>
        {(provided, snapshot) => (
          <div ref={provided.innerRef} {...provided.droppableProps}
            className={`flex-1 overflow-y-auto px-2.5 py-2.5 column-scroll transition-colors duration-200
